...
function createColaboradoresStore() {
	...
	return {
		...
		add: (colaborador: Colaborador) => update((lista) => [...lista, colaborador]),

		update: (id: string, dados: Partial<Colaborador>) =>
			update((lista) => lista.map((c) => (c.id === id ? { ...c, ...dados } : c))),

		remove: (id: string) => update((lista) => lista.filter((c) => c.id !== id)),
