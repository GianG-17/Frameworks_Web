  ...
  <div class="modal-backdrop" onclick={handleBackdrop}>
    <form class="modal" role="dialog" aria-modal="true" onsubmit={handleSubmit}>
      ...
      <div class="modal-body">
        <p class="aviso">
          A batida <strong>não será apagada</strong>. Será criado um registro de anulação
          paralelo, conforme exige a Portaria 671/2021. A batida continuará visível no
          histórico marcada como anulada.
        </p>

        <dl class="info">
          <div><dt>Tipo</dt><dd>{tipoLabel[punch.type] ?? punch.type}</dd></div>
          <div><dt>Data/hora</dt><dd>{formatHora(punch.timestamp)}</dd></div>
          <div><dt>Origem</dt><dd>{punch.method === 'manual' ? 'Manual' : 'QR Code'}</dd></div>
        </dl>
...
<style>
  ...
  .btn-fechar {
    ...
    color: #64748b; cursor: pointer; padding: 0.25rem;
  }
  .modal-body { padding: 1.25rem 1.5rem; display: flex; flex-direction: column; gap: 1rem; }
  .aviso {
    font-size: 0.825rem; color: #475569;
    background: #fef2f2; border-left: 3px solid #ef4444;
    padding: 0.625rem 0.75rem; border-radius: 0.375rem; margin: 0;
    line-height: 1.4;
  }
  .info {
  ...
  .info > div { display: flex; gap: 0.5rem; }
  .info dt { color: #64748b; min-width: 90px; margin: 0; }
  .info dd { margin: 0; color: #111; font-weight: 500; }
  .campo { display: flex; flex-direction: column; gap: 0.375rem; }
  label { font-size: 0.875rem; font-weight: 500; color: #111; }
  textarea {
    padding: 0.5rem 0.75rem; border: 1.5px solid #e2e8f0;
    border-radius: 0.5rem; font-size: 0.9rem; background: #f8fafc;
    font-family: inherit;
  }
  textarea:focus {
  ...
  .erro {
    ...
    padding: 0.5rem 0.75rem; border-radius: 0.5rem; font-size: 0.825rem;
  }
  .modal-footer {
    display: flex; justify-content: flex-end; gap: 0.75rem;
    padding: 0.75rem 1.5rem 1.25rem; border-top: 1px solid #e2e8f0;
  }
  .btn {
    padding: 0.5rem 1.125rem; border-radius: 0.5rem; font-size: 0.9rem;
    font-weight: 500; cursor: pointer; border: none;
  }
  .btn:disabled { opacity: 0.6; cursor: not-allowed; }
  .btn--secundario { background: #e2e8f0; color: #111; }
