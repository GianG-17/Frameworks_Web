...
function diaSemanaKey(date: Date): keyof DiaConfig {
  ...
  return ['domingo', 'segunda', 'terca', 'quarta', 'quinta', 'sexta', 'sabado'][idx] as keyof DiaConfig;
}
...
const colaboradoresSeed: ColaboradorSeed[] = [
  ...
  },
  {
    ...
    jornada: 'comercial',
    rngSeed: 303,
    comportamento: { faltaProb: 0.0, atrasoGraveProb: 0.10, esqueceBatidaProb: 0.05 },
...
async function main() {
  ...
  const meioPeriodo = await prisma.jornada.create({
    data: { empresaId: empresa.id, nome: 'Meio Período Manhã', dias: JSON.stringify(jornadaMeioPeriodo) }
  });
